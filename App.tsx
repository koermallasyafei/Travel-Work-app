...
const PlannerItemCard: React.FC<{ item: PlannerItem }> = ({ item }) => {
    ...
    return (
        <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-4">
            <h3 className="font-bold text-md text-slate-200">{ICONS[item.type]} {item.name || item.title}</h3>
...
const App: React.FC = () => {
  ...
  const addToPlanner = (item: Stay | WorkSpot | CommunityEvent, type: 'Stay' | 'WorkSpot' | 'Community') => {
    setPlannerItems(prev => [...prev, { ...item, type }]);
  };
        ...
        case 'FindStay':
            return (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 p-6">
                    {MOCK_STAYS.map(stay => (
                        <StayCard key={stay.id} stay={stay} onAddToPlanner={() => addToPlanner(stay, 'Stay')} />
                    ))}
                </div>
            );
        case 'WorkSpots':
             return (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 p-6">
                    {MOCK_WORK_SPOTS.map(spot => (
                        <WorkSpotCard key={spot.id} spot={spot} onAddToPlanner={() => addToPlanner(spot, 'WorkSpot')} />
                    ))}
                </div>
            );
        case 'Community':
            return (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 p-6">
                    {MOCK_COMMUNITY_EVENTS.map(event => (
                        <CommunityEventCard key={event.id} event={event} onAddToPlanner={() => addToPlanner(event, 'Community')} />
                    ))}
                </div>
            );
        case 'Planner':
